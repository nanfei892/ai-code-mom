...
import com.zsm.aicodemom.ai.AiCodeGenTypeRoutingService;
import com.zsm.aicodemom.ai.AiCodeGenTypeRoutingServiceFactory;
...
import com.zsm.aicodemom.core.handler.StreamHandlerExecutor;
...
import com.zsm.aicodemom.service.ScreenshotService;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App> implements AppService {
    ...
    public String deployApp(Long appId, User loginUser) {
        ...
        App app = this.getById(appId);
        ThrowUtils.throwIf(app == null, ErrorCode.NOT_FOUND_ERROR, "应用不存在");
        // 3.验证应用是否具有权限部署该应用，仅本人可部署
        if (!app.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "无权限部署该应用");
        }
        // 4. 检查是否已有 deployKey
        String deployKey = app.getDeployKey();
        // 4.1 如果没有 deployKey，则生成6为大小写字母+数字
    ...
    public void generateAppScreenshotAsync(Long appId, String appUrl) {
        ...
        Thread.startVirtualThread(() -> {
            // 调用截图服务生成截图并上传
            String screenshotUrl = screenshotService.generateAnUploadScreenshot(appUrl);
            // 更新应用封面字段
            App updateApp = new App();
            updateApp.setId(appId);
