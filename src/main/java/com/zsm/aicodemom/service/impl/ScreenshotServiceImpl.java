...
import com.zsm.aicodemom.manager.CosManager;
import com.zsm.aicodemom.service.ScreenshotService;
import com.zsm.aicodemom.utils.WebScreenshotUtils;
...
public class ScreenshotServiceImpl implements ScreenshotService {
     ...
     */
    @Override
    public String generateAnUploadScreenshot(String webUrl) {
        ThrowUtils.throwIf(StrUtil.isBlank(webUrl), ErrorCode.PARAMS_ERROR, "网页URL不能为空");
        log.info("开始生成网页截图，URL：{}", webUrl);
        // 1. 生成本地截图
        String localScreenshotPath = WebScreenshotUtils.saveWebPageScreenshot(webUrl);
        ThrowUtils.throwIf(StrUtil.isBlank(localScreenshotPath), ErrorCode.OPERATION_ERROR, "本地截图保存失败");
        try {
            // 2. 上传到对象存储
     ...
     * @param fileName 文件名
     * @return 对象存储的键
     * 格式：/screenshots/2026/03/23/fileName.jpg
     */
